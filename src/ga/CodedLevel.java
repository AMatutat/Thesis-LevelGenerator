...
public class CodedLevel {
    ...
    public CodedLevel copyLevel() {
        ...
        for (int x = 0; x < this.xSize; x++)
            for (int y = 0; y < this.ySize; y++) r.changeField(x, y, this.level[x][y]);
        return r;
    }
    ...
    /** @return Liste aller gepr�ften W�nde */
    public ArrayList<String> getCheckedWalls() {
        return this.checkedWalls;
    }

    /** @return Liste aller erreichbaren B�den */
    ...
    /** @return H�he des Level */
    public int getYSize() {
        return this.ySize;
    }
    /** @return Ausgangs Position */
    public Point getExit() {
        if (hasExit()) return this.exit;
        return null;
    }
    /** @return Start Position */
    public Point getStart() {
        if (hasStart()) return this.start;
        return null;
    }
    /** @return Levelaufbau */
    ...
    public void changeField(int x, int y, char s) {
        if (this.level[x][y] == Reference.REFEERNCE_EXIT) this.exit = null;
        else if (this.level[x][y] == Reference.REFERENCE_START) this.start = null;

        if (s == Reference.REFEERNCE_EXIT) {
            if (this.hasExit()) s = Reference.REFERENCE_FLOOR;
            else {
                this.exit = new Point(x, y);
            }
        } else if (s == Reference.REFERENCE_START) {
            if (this.hasStart()) s = Reference.REFERENCE_FLOOR;
            else {
    ...
    public void printLevel() {
        for (int y = 0; y < this.ySize; y++) {
            for (int x = 0; x < this.xSize; x++) System.out.print(level[x][y]);
            System.out.println("");
        }
        System.out.println("");
    }
